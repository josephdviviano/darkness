 ...
 * start() on becoming a new top state, suspend() when a new state covers this
 * as new active, resume() when state becomes active again, and exit() when the
 * state is being removed from the queue.
 * @note To give the control to another state, when exiting this one, just
 * pushState the new state in the exit() method
 */
class GameState {
public:
    /** Constructor */
    ...
    /** Do a state startup. Called if the state becomes the new active game
     * state (added to the state stack) */
    ...
    /** Do a state resume. Called when the state becomes active again, being not
