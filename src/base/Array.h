 ...
 * @todo Introduce iterators over the contents (wrappers around the array member
 * pointers)
 */
...
public:
    ...
    Array()
        : mMinIndex(1), mMaxIndex(-1), mNegativeArray(NULL),
          mPositiveArray(NULL) {
    ...
    /// Grows the array. New sizes have to be greater the old - so no objects
    /// are removed
...
public:
    ...
    /// Grows the array. New sizes have to be greater the old - so no objects
    /// are removed
    void grow(size_t newSize) { growBuf(newSize); }
