...
unsigned char **ManualFonFileLoader::ReadFont(int *ResultingColor) {
    ...
    *mFontFile >> FontHeader;
    ...
    // No precalc. Let's organise the Font into groups of 16, the resolution
    // will come out
    ...
    // If the process did not finish with the 16th char in the row, try to
    // update the width
    if (X > ImageWidth)
        ImageWidth = X;
    ...
    if (FontHeader.Format == 0) {
        ...
        for (N = 0; N < FontHeader.NumRows; N++) {
            for (CharInfoList::const_iterator It = mChars.begin();
                 It != mChars.end(); It++) {
                const CharInfo &Char = *It;
                Y = Char.Column;
                for (X = 0; X < Char.Width; Y++, X++)
                    RowPointers[Char.Y + N][Char.X + X] =
                        ((Ptr[Y / 8] >> (7 - (Y % 8))) & 1) ? WHITE_INDEX
                                                            : BLACK_INDEX;
            }
        ...
        Ptr = BitmapData;
        for (N = 0; N < FontHeader.NumRows;
             N++) { // Scanline of the font character...
            for (CharInfoList::const_iterator It = mChars.begin();
                 It != mChars.end(); It++) {
                const CharInfo &Char = *It;
...
RGBQuad *ManualFonFileLoader::ReadPalette() {
    ...
    if ((mPaletteType == ePT_PCX) ||
        (mPaletteType == ePT_DefaultBook)) { // PCX file specified...
        RGBQuad *Palette = new RGBQuad[256];
        ...
        if (Manuf != 0x0A ||
            ...
            LogManager::getSingleton().logMessage(
                "Invalid palette file specified - seems not to be a PCX file!");
            return (RGBQuad *)
                ColorTable; // Should not matter - the cast (if packed)
        }
        ...
        if ((BPP == 8) &&
            ...
            delete[] Palette; // Clean up!
            LogManager::getSingleton().logMessage(
                "Invalid palette file specified - not 8 BPP or invalid "
                "Padding!");
...
//-------------------------------------------------------------------
int ManualFonFileLoader::WriteImage(RGBQuad *ColorTable,
    ...
    mBmpFileSize = sizeof(BITMAPFILEHEADER) + sizeof(BitmapInfoHeader) +
                   (sizeof(RGBQuad) * 256) + (mImageDim * mImageDim);
...
int ManualFonFileLoader::LoadDarkFont() {
    ...
    if (mPaletteType != ePT_Default) {
        ...
        if (!PaletteData) {
            LogManager::getSingleton().logMessage(
                "Could not load palette data, defaulting to default palette");
            // return -2;
...
//-------------------------------------------------------------------
void ManualFonFileLoader::createOgreTexture(unsigned char **Img,
                                            RGBQuad *Palette) {
    // Create a texure, then fill it
    TexturePtr Txt = TextureManager::getSingleton().createManual(
        mTxtName, mFontGroup, TEX_TYPE_2D, mImageDim, mImageDim, 1,
        PF_A8R8G8B8);
...
int ManualFonFileLoader::CreateOgreFont(Font *DarkFont) {
    ...
    for (CharInfoList::const_iterator It = mChars.begin(); It != mChars.end();
         It++) {
        const CharInfo &Char = *It;
        DarkFont->setGlyphTexCoords(
            Char.Code, (float)(Char.X - 2) / mImageDim,
            (float)(Char.Y - 2) / mImageDim,
            (float)(Char.X + Char.Width + 2) / mImageDim,
            (float)(Char.Y + mNumRows + 2) / mImageDim, 1.0);
    }

    DarkFont->setParameter(
        "size",
        StringConverter::toString(
            mNumRows)); // seems mNumRows (NumRows) means Y size of the font...
...
void ManualFonFileLoader::loadResource(Resource *resource) {
        ...
        if ((mPaletteType == ePT_DefaultBook) ||
            (mPaletteFileName != "" &&
             ((mPaletteType == ePT_PCX) ||
              (mPaletteType == ePT_External)))) { // Palette file specified
            if (mPaletteType == ePT_DefaultBook) {
                ...
                if (SlashPos != String::npos)
                    mPaletteFileName =
                        BaseName.substr(0, SlashPos + 1); // Include the slash
                mPaletteFileName += "BOOK.PCX";
            }
            LogManager::getSingleton().logMessage(
                "Font DEBUG: Will load a custom file palette");
        } else {
            LogManager::getSingleton().logMessage(
                "Non-Default palette type, but no filename specified "
                "(Defaulting to LG's default pal). Font name " +
                BaseName);
        }
    }
    ...
    Ogre::DataStreamPtr Stream =
        Ogre::ResourceGroupManager::getSingleton().openResource(
            BaseName, mFontGroup, true, resource);
    mFontFile = FilePtr(new OgreFile(Stream));

    if (LoadDarkFont())
        LogManager::getSingleton().logMessage(
            "An error occurred while loading the font " + BaseName);

    if (CreateOgreFont(DarkFont))
        LogManager::getSingleton().logMessage(
            "An error occurred creating Ogre font");
}
} // namespace Ogre
