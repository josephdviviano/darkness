...
public:
     ...
     * @note This function does not return a clone to the file, rather always
     * the file itself. Usage at two places simultaneously will cause an
     * unexpected seek behavior
    ...
    virtual FilePtr createFile(const std::string &name, uint32_t ver_maj,
                               uint32_t ver_min) = 0;
    ...
    @warning Do not use the source file as destination. The source file data are
    read on demand, so owerwriting the source is not a good idea
    @note the above limitation should not cause any complications, as all the
    standard writing is done into an empty file group */
    virtual void write(FilePtr &dest) = 0;

    /** Get the File name of this database, or empty string if no underlying
     * file is used */
    std::string getName();
...
* This class exposes the chunks of that format as separate files, which can be
