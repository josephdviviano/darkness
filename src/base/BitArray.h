...
public:
     ...
     * The given buffer has to be allocated so it can hold up getByteSize()
     * bytes
     */
    ...
    /// grows the bitArray to be able to index new min->max indices, preserving
    /// the previous values
    ...
    public:
        ...
        operator bool() const { return mArray.getBit(mIndex); }
...
protected:
    ...
    inline size_t getIndex(int addr) const { return size_t(addr) >> 3; }

    /// Bit offset for address getter
    inline int getOffset(int addr) const { return size_t(addr) & 0x07; }

    /// Helper size getter for index - needed byte size for array to hold
    /// certain index
