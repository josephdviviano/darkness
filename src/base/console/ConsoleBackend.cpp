...
ConsoleBackend::ConsoleBackend(unsigned int text_history)
    : mCommandMap(), mCompletionMap(), mTextHistory(text_history) {
    mCommandMap.clear();
...
void ConsoleBackend::registerCommandListener(std::string command,
                                             ConsoleCommandListener *listener) {
    std::map<string, ConsoleCommandListener *>::iterator commandIt =
        mCommandMap.find(command);

    if (commandIt != mCommandMap.end()) { // already registered
        LOG_DEBUG("ConsoleBackend::registerCommandListener: Command %s is "
                  "already registered, reregistering the listener pointer",
                  command.c_str());
...
void ConsoleBackend::executeCommand(std::string command) {
    ...
    if (space_pos != string::npos) {
        ...
        command_parameters =
            command.substr(space_pos + 1, command.length() - (space_pos + 1));
    }

    // Try if it is a in-built command
    if (command_part == "commands" || command_part == "?") {
        std::map<string, ConsoleCommandListener *>::iterator commands =
            mCommandMap.begin();

        for (; commands != mCommandMap.end(); commands++) {

            // Try to look for a hint text
            std::map<string, string>::const_iterator hintIt =
                mHintMap.find(commands->first);
...
void ConsoleBackend::pullMessages(std::vector<ConsoleBackend::Message> &target,
                                  int pos, unsigned int lines) {
