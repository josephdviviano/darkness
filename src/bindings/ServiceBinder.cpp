...
PyObject *ServiceBinder::getConfigService(PyObject *self, PyObject *args) {
    try {
        ...
        PyErr_Format(PyExc_EnvironmentError,
                     "Exception happened while getting service: %s",
                     e.getDetails().c_str());
        return NULL;
    }
}
...
PyObject *ServiceBinder::init(PyObject *container) {
    ...
    // init the services (we init all, they register into the module, enabling
    // pydoc usage on them)
    ...
    PyModule_AddIntConstant(module, "SERVICE_LINK_LISTENER",
                            SERVICE_LINK_LISTENER);
    PyModule_AddIntConstant(module, "SERVICE_PROPERTY_LISTENER",
                            SERVICE_PROPERTY_LISTENER);
    PyModule_AddIntConstant(module, "SERVICE_OBJECT_LISTENER",
                            SERVICE_OBJECT_LISTENER);
    PyModule_AddIntConstant(module, "SERVICE_DATABASE_LISTENER",
                            SERVICE_DATABASE_LISTENER);
    PyModule_AddIntConstant(module, "SERVICE_INPUT_LISTENER",
                            SERVICE_INPUT_LISTENER);
