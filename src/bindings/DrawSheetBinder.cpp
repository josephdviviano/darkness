...
PyObject *DrawSheetBinder::addDrawOperation(PyObject *self, PyObject *args) {
    ...
    if (PyArg_ParseTuple(args, "O", &dop)) {
        // Either it is RenderedImage or RenderedLabel. DrawOperation should
        // extract, Anyway
        DrawOperation *dopc;
        ...
        PyErr_SetString(PyExc_TypeError,
                        "Expected a DrawOperation as an argument!");
        return NULL;
    }
...
// ------------------------------------------
PyObject *DrawSheetBinder::setResolutionOverride(PyObject *self,
                                                 PyObject *args) {
    __PYTHON_EXCEPTION_GUARD_BEGIN_;
...
bool DrawSheetBinder::extract(PyObject *object, DrawSheetPtr &sheet) {
    ...
    // to overcome this, we have a casting method embedded in the type tree
    // where needed (so it's transparent here)
