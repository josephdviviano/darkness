...
/**
 ...
 @brief A material service implementation. Handles terrain/object materials -
 loading, unloading, object service properties of them.
 */
...
public:
     ...
     */
    Ogre::MaterialPtr getWRMaterialInstance(unsigned int texture, int tag,
                                            unsigned int flags);

    typedef std::pair<Ogre::uint, Ogre::uint> TextureDimensions2D;

    /** Getter for WR texture size (after scaling with the custom material
     * parameters)
     * @param texture The wr texture id
     */
    TextureDimensions2D getTextureDimensions(unsigned int texture);

    /** Prepares a single TextureUnitState filled with all the animation frames
     * of the loaded image set. Searches for all images that have the same image
     * name, or have a _NUMBER added to the filename. If none additional
     * textures found, the result is a steady texture specified.
     *
     * @param skeleton The skeletal material to fill with the animation/static
     * texture
     * @param baseTextureName The base name of the texture, incl. the extension
     * @param fps The desired FPS of the animation
     * @return TextureUnitState pointer which was filled with the
     * animation/static texture
     * */
    Ogre::TextureUnitState *
    createAnimatedTextureState(Ogre::Pass *pass,
                               const Ogre::String &baseTextureName,
                               const Ogre::String &resourceGroup, float fps);
...
protected:
    ...
    /** Called by loadMaterials. Load the FLOW_TEX and initializes
     * \@templateXXXX according to IN and OUT texture numbers and names. Needs
     * material definitions
     * named water/AAAA_in and water/AAAA_out where AAAA is the expected flow
     * texture name (usualy bl, gr, l2, l3, l4) */
    void loadFlowTextures(const FileGroupPtr &db);

    /** Internal method which loads the textures from the dark database
     * definitions. Families, textures and flowgroups Texture preparation -
     * prepares materials in the form of \@templateXXXX where XXXX is the
     * texture number */
    void loadMaterials(const FileGroupPtr &db);

    /** A fail-back method, which construct a default-valued material, if a
     * material script for a certain family/texture was not found */
    void createStandardMaterial(unsigned int idx, std::string matName,
    ...
    /// Gets the material instance for the given material slot index and tag
    /// (autocreates, but with no modifications - clone only)
    Ogre::MaterialPtr getWorldMaterialInstance(unsigned int idx, int tag);

    /// modifies the material to contain lightmap
    void prepareMaterialInstance(Ogre::MaterialPtr &mat, unsigned int idx,
                                 int tag);

    /** Creates a vector containing all accessible animated textures in a
     * sequence. This means that given Eng_3/GOO.PCX, this method will return
    ...
    /// Map of Texture sizes (this is needed for the WR texturing to work) for
    /// WR
    ...
    /// map of the texture sizes (fam/texture -> size). TODO: Evaluate the
    /// possibility to always scale to predefined dimensions...
