...
* @todo Object queries - getAllConcrete, getAllArchetype, getMetapropertiesOf,
etc. ObjectQuery class introduced
* @todo FindClosestObjectNamed (Based on archetype name!), RenderedThisFrame
(based on entity listener. This one should be implemented in RenderService)
* @todo Transient objects - are those needed? If so, implement (This one will be
implemented on demand, as I currently can't see a reason to do those)
* @todo inheritsFrom - good to have query. Should only search archetypes, not
metaprops
* @todo OBJECT_CREATE_STARTED message type for those services needing a
preparation for object creation (for example render service will create a
...
public:
    ...
    /** Creates a new concrete object, inheriting from archetype archetype, and
    returns it's id
    ...
    */
    ...
    /// Good to use when wanting to prevent broadcasting of the creation before
    /// the properties/links are set
    int beginCreate(int archetype);

    /// finalises the creation of the given object (only broadcasts that object
    /// was created)
    ...
    /// @returns Object's position or 0,0,0 if the object is abstract or has no
    /// position yet (while loading f.e.)
    Vector3 position(int objID);

    /// @returns Object's orientation or Quaternion::IDENTITY if the object has
    /// no orientation
    ...
    */
     ...
     * @param relative If true, the position and orientation are taken as
     * differences rather than absolute positions
     */
    void teleport(int id, const Vector3 &pos, const Quaternion &ori,
                  bool relative);
     ...
     * @return true if the object inherits from the given metaproperty, false
     * otherwise
     */
    bool hasMetaProperty(int id, const std::string &mpName);

    /** Grows the whole object system to allow the storage of the given range of
...
protected:
    ...
    /// Begins the creation of an object. Prepares properties and links as
    /// needed
    ...
    /// Destroys object, frees accompanying links and properties, broadcasts the
    /// change
    void _destroyObject(int objID);

    /// Prepares for object. Does necessary steps to fully accept the object.
    /// informs all the listeners that object creation starts (f.e. creates
    /// SceneNode for concrete objects)
    void _prepareForObject(int objID);

    /// Returns a new free archetype ID. Modifies the mMinID/mMaxID as
    /// appropriate, removes the ID from the free id's
    int getFreeID(bool archetype);

    /// Frees object ID for later use
    void freeID(int objID);

    /// Resets the min and max id's to some sane values (uses config service
    /// values obj_min and obj_max if found)
    void resetMinMaxID();
