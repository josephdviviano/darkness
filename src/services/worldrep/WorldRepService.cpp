...
/*----------------------------------------------------*/
template <>
const size_t ServiceImpl<WorldRepService>::SID = __SERVICE_ID_WORLDREP;
...
void WorldRepService::setSkyBox(const FileGroupPtr &db) {
    ...
    if (skyChunk) { // Thief1 sky. Thief2 has NewSky. Will need to make a custom
                    // scene node for this.
        uint32_t skyMode;
...
void WorldRepService::createBSP(unsigned int BspRows, WRBSPNode *tree) {
    ...
    for (unsigned int i = 0; i < BspRows; i++) {
        ...
        if (_BSP_FLAGS(wr_node) &
            0x04) { // inverted plane normal, swap front n' back child
            if (front >= 0)
...
const std::string &WorldRepServiceFactory::getName() { return mName; }
