...
struct WRCellHeader {    // SIZE: 31
    ...
    uint8_t numPortals;  // faces that define a portal... Count this number from
    ...
    uint16_t polymapSize;  // this is repeated at the start of the polygon index
                           // list. you could do a sanity-check against it [TNH]
    uint8_t numAnimLights; // number of animated lights - animlm indexes length
    ...
    Vector3 center;
    float radius; // Only an approximation, but enough to guarantee that every
...
struct WRPolygon {    // SIZE: 8
    ...
    uint8_t unk2; // Some flags... 41, FF... etc... (0x010 might be light map
                  // only render)
...
struct WRPolygonTexturing { // SIZE: 12+12+12+12 = 48
    ...
    Vector3 axisV; // V axis - both directions of texture growth (e.g. U axis
                   // and V axis) - and they are not normalised! (in some way
                   // related to scale)

    int16_t u; // txt shift u (must divide by 1024 to get float number (and I
               // dunno why, I had to invert it too))
    int16_t v; // txt shift v

    uint8_t txt;          // texture number (index to the texture list)
    uint8_t originVertex; // the vertex index of the origin vertex - the vertex
    ...
    friend File &operator<<(File &st, const WRPolygonTexturing &ch) {
        st << ch.axisU << ch.axisV << ch.u << ch.v << ch.txt << ch.originVertex
           << ch.unk << ch.scale << ch.center;
        return st;
    }
...
struct WRLightInfo { // SIZE: 4+4+12 = 20
    int16_t u; // LMAP U shift probably (if, then the same approach as in the
               // wr_face_info_t)
    ...
    uint32_t animflags; // map of animlight lightmaps present - bit 1 means yes
...
struct WRBSPNode { // 20b - BSP node
    ...
    int32_t plane;  // if non-leaf, the splitting cells plane number - e.g. read
                    // nodes[cell].plane[plane] to get the plane you want
