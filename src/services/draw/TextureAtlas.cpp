...
TextureAtlas::TextureAtlas(DrawService *owner, DrawSource::ID id)
    ...
    // Here, we insert the white 2x2 texture into the draw sources. It is always
    // there, so we have a direct pointer to it in the atlas as well. it is used
    // for texture-less objects (Governed by vertex colour alone) Inspiration
    // was taken from Canvas, as with other things image:
    ...
    mVertexColour->getImage()->loadDynamicImage(
        reinterpret_cast<Ogre::uchar *>(pixels), 2, 2, 1, Ogre::PF_BYTE_BGRA,
        false);
...
//------------------------------------------------------
DrawSourcePtr TextureAtlas::createDrawSource(const Ogre::String &imgName,
...
void TextureAtlas::build() {
    ...
    do {
        ...
        while (it != mMyDrawSources.end()) {
            ...
            LOG_VERBOSE("TextureAtlas: (%s) Trying to place %d x %d (%d -> %d)",
                        mAtlasName.c_str(), ps.width, ps.height,
    ...
    LOG_INFO("TextureAtlas: (%s) Creating atlas with dimensions %d x %d",
    ...
    // We'll iterate over all draw sources, painting the pixels onto the
    // allocated space
    DrawSourceList::iterator it = mMyDrawSources.begin();
    ...
    while (it != mMyDrawSources.end()) {
        ...
        if (img->getFormat() != Ogre::PF_BYTE_BGRA) {
            conversionBuf = new unsigned char[img->getWidth() *
                                              img->getHeight() * pixelsize];
            Ogre::PixelBox convPixels(Ogre::Box(0, 0, dps.width, dps.height),
                                      Ogre::PF_BYTE_BGRA, conversionBuf);
        ...
        // TODO: we're always handling 32bit data, so we could as well transfer
...
void TextureAtlas::enlarge(size_t area) {
    ...
    LOG_DEBUG("TextureAtlas: (%s) Enlarging atlas from %d x %d",
              mAtlasName.c_str(), mAtlasSize.width, mAtlasSize.height);

    size_t size;

    // a trick to note - always enlarged at least once, but can be more times to
    // fit at least the area specified
    do {
        // enlarge
...
void TextureAtlas::prepareResources() {
    mTexture = Ogre::TextureManager::getSingleton().createManual(
        mAtlasName, Ogre::ResourceGroupManager::DEFAULT_RESOURCE_GROUP_NAME,
        Ogre::TEX_TYPE_2D, mAtlasSize.width, mAtlasSize.height, 1,
        Ogre::PF_BYTE_BGRA, Ogre::TU_STATIC_WRITE_ONLY);
...
void TextureAtlas::dropResources() {
    if (mTexture) {
        ...
        if (tus) {
            int index = mMaterial->getTechnique(0)
                            ->getPass(0)
                            ->getTextureUnitStateIndex(tus);
            mMaterial->getTechnique(0)->getPass(0)->removeTextureUnitState(
                index);
        }
