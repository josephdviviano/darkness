...
DrawSource::DrawSource(DrawService *owner)
    : DrawSourceBase(), mAtlassed(false), mImageLoaded(false), mOwner(owner),
      mPixmap(NULL) {
...
//------------------------------------------------------
DrawSource::DrawSource(DrawService *owner, const Ogre::String &name,
                       const Ogre::String &group,
                       const Ogre::MaterialPtr &extMaterial)
    : DrawSourceBase(), mAtlassed(false), mImageLoaded(false), mOwner(owner),
      mPixmap(NULL) {
...
//------------------------------------------------------
void DrawSource::atlas(const Ogre::MaterialPtr &mat, size_t x, size_t y,
                       size_t width, size_t height) {
    mMaterial = mat;
    mDisplacement =
        Ogre::Vector2((Ogre::Real)x / width, (Ogre::Real)y / height);
    mSize = Ogre::Vector2((Ogre::Real)mPixelSize.width / width,
                          (Ogre::Real)mPixelSize.height / height);
    mAtlassed = true;
}
...
//------------------------------------------------------
void DrawSource::loadImage(const Ogre::String &name,
                           const Ogre::String &group) {
