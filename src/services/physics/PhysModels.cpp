...
/*---------------------- PhysModels ------------------*/
/*----------------------------------------------------*/
PhysModels::PhysModels(PhysicsService *owner)
    : mOwner(owner){
...
void PhysModels::addToStationary(PhysModel *mdl) {
    STUB_WARN_TXT(
        "Does not handle any notification, even model is not informed");
...
void PhysModels::write(const FilePtr &tag, unsigned int physVersion) {
    STUB_WARN();
    // if the object is found in the stationary list, it is only written in the
    // second pass
};
