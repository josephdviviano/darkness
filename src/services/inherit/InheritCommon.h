 ...
 * values on object ID's
 */
typedef enum {
    /// Object gained an inherited value, did not implement before
    INH_VAL_ADDED = 1,
    /// Inheritance of the value was canceled (object stopped implementing a
    /// value)
    INH_VAL_REMOVED,
    /// Object changed the inheritance source, did implement before
    INH_VAL_CHANGED,
    /// Object changed a certain field's value (effectively - not masked by
    /// metaprops), inheritance did not change
...
struct InheritValueChangeMsg {
    ...
    /// (ignore for INH_VAL_FIELD_CHANGED)
    int srcID;
 ...
 * This means that all the objects that are changed by the inheritance
 * modification can be notified through the listener */
...
public:
    ...
    /** Manual method for setting that a certain object ID directly implements
     * (or not) the inherited quality */
    virtual void setImplements(int objID, bool impl) = 0;

    /** Returns the effective object ID for the given object ID
     * @note The effective object is the object holding the used value for the
     * object ID given
     * @return The effective object ID, or 0, if no effective is found (the
     * object itself is effective) */
    virtual int getEffectiveID(int srcID) const = 0;

    /** The core inheritance describing method.
     * This method is used to validate the given inheritance situation to be
     * acceptible or not. An example:
     * @code
     ...
     * }
     * @endcode
     * @note For realy special kind of inheritors, overriding the refresh method
     * is a good alternative
     * @param srcID The source object ID
     * @param dstID The destination object ID
     * @return true if the source is valid, false if the source is not valid
     * inheritance source */
    virtual bool validate(int srcID, int dstID,
                          unsigned int priority) const = 0;

    /// Used upon total cleanout of the database
    virtual void clear() = 0;

    /// grows the internal tables of the inheritor to allow the storage of data
    /// about defined object range
